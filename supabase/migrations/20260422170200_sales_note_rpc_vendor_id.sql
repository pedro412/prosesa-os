-- LIT-107: extend create_sales_note to accept vendor_id in the payload.
--
-- Adds one declaration, one extraction, and one column in the
-- sales_notes INSERT. No validation on the id — the FK constraint
-- handles referential integrity, and the POS picker only surfaces
-- active vendors, so "pick a vendor that doesn't exist anymore" can't
-- happen through the normal path. Callers that forge a payload with
-- an invalid id get a clean FK violation.
--
-- vendor_id is optional. null means either walk-in-closed-by-Gustavo
-- or Gustavo-forgot-to-attribute. Both legitimate; display "Sin
-- vendedor" in the UI.
...
begin
  ...
  v_vendor_id := nullif(payload ->> 'vendor_id', '')::uuid;
  ...
  insert into public.sales_notes (
    company_id, customer_id, vendor_id, status,
    subtotal, iva, total,
    iva_rate_snapshot, iva_inclusive_snapshot,
    requires_invoice, notes
  )
  values (
    v_company_id, v_customer_id, v_vendor_id, 'pendiente',
