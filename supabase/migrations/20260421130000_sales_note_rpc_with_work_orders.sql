-- LIT-37 / M4-2 — Extend create_sales_note to persist work_orders in
-- the same transaction as the nota, lines, and payments.
--
-- Model recap (CLAUDE.md §7/§8):
--   * 1 nota → 0..N work_orders.
--   * Per-line `work_order_client_id` routes the line: null → counter
--     line, non-null → attached to the work order with that client id.
--   * Orders with zero referencing lines are dropped silently (the UI
--     already drops them, this is belt-and-suspenders).
--
-- Payload additions:
--   {
--     "lines": [{ ..., "work_order_client_id": "order-1" | null }],
--     "work_orders": [
--       { "client_id": "order-1",
--         "description": "...",
--         "priority": "normal" | "urgente",
--         "promised_at": "2026-05-01T12:00:00Z" | null }
--     ]
--   }
--
-- Return shape changes from `(id uuid, folio text)` to
-- `(id uuid, folio text, work_orders jsonb)`, where work_orders is an
-- array of `{ id, folio, client_id }`. Because this changes the
-- declared return type, Postgres requires a DROP before CREATE.
--
-- Insertion order inside the transaction:
--   1. sales_notes  (parent folio derived by existing trigger)
--   2. work_orders  (derived folios via LIT-38's generate_work_order_folio)
--   3. sales_note_lines  (work_order_id mapped from client_id → wo.id)
--   4. payments  (unchanged; LIT-97 partial-payment behavior preserved)

drop function if exists public.create_sales_note(jsonb);

create function public.create_sales_note(payload jsonb)
...
declare
  ...
  v_wo_id uuid;
  v_wo_folio text;
  v_wo_declared_ids jsonb := '{}'::jsonb;  -- set-as-object: { client_id: true }
  v_wo_map jsonb := '{}'::jsonb;            -- { client_id: { id, folio } }
  v_wo_result jsonb := '[]'::jsonb;         -- [ { id, folio, client_id } ]
...
begin
  ...
  -- Validate work_orders[].client_id: required, non-empty, unique.
  -- Also sanity-check priority / promised_at so the insert doesn't
  -- surface a cryptic CHECK violation later.
  for v_wo_elem in select elem from jsonb_array_elements(v_work_orders) as elem loop
    ...
    -- Defer the timestamptz cast until insert time; nullif-empty here
    -- so a blank string doesn't trip the cast.
    perform nullif(v_wo_elem ->> 'promised_at', '')::timestamptz;
  end loop;

  -- Validate every line's work_order_client_id (if present) points to
  -- a declared order. Catches typos and stale client refs before we
  -- start writing rows.
  ...
  -- Insert work_orders, skipping orphans (zero referencing lines).
  -- Folio is derived by work_orders_assign_folio trigger (LIT-38).
  for v_wo_elem in select elem from jsonb_array_elements(v_work_orders) as elem loop
