-- LIT-97 — Allow anticipo (partial payment) on counter sales.
--
-- The previous definition of `create_sales_note` rejected any payload
-- where 0 < sum(payments) < total, forcing counter sales to be
-- committed only when fully paid. Karina flagged this during QA of
-- LIT-95 (point 4): CLAUDE.md §8 says anticipo must accept any amount
-- from 0 to 100% of total, with the saldo tracked until full payment.
--
-- This migration redefines the RPC without the total-coverage raise.
-- The existing `payments_recompute_note_status` trigger already lands
-- the correct derived status:
--   * paid_sum = 0       → 'pendiente'
--   * 0 < paid_sum < total → 'abonada'
--   * paid_sum >= total  → 'pagada'
--
-- Everything else stays intact: per-row method/amount/card_type
-- validation, IVA computation, line insertion, folio generation.
